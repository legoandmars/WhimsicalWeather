...
uniform float _IsGamma = 0;

// modify to use hue 0-1 so i don't have to think as much
static const float CUSTOM_PI = 3.1415926535897932384626433832795;
static const float hueMultiplier = (CUSTOM_PI * 2) / 1;
...
// TODO: Do the real calculations from these instead of just the estimate
float3 RemoveGammaIfNeeded(float3 rgb)
{
    return lerp(rgb, float3(pow(rgb.r, 1/2.2), pow(rgb.g, 1/2.2), pow(rgb.b, 1/2.2)), _IsGamma);
}

float3 AddGammaIfNeeded(float3 rgb)
{
    return lerp(rgb, float3(pow(rgb.r, 2.2), pow(rgb.g, 2.2), pow(rgb.b, 2.2)), _IsGamma);
}
...
{
    // truncated to remove gamut stuff
...
float3 RGBtoOKHSL(float3 col) {
    float3 lab = linear_srgb_to_oklab(AddGammaIfNeeded(col));
    ...
    float L = lab.r;
    float h = 0.5 + 0.5*atan2(-lab.b, -lab.g)/CUSTOM_PI;
...
float3 OKHSLtoRGB(float3 hsl)
{
    ...
    float a_ = cos(2*CUSTOM_PI*h);
    float b_ = sin(2*CUSTOM_PI*h);   
    float L = ToeInv(l);
    ...
    return RemoveGammaIfNeeded(rgb);
}
